...
class CustomImGui : public ui::UI {
  public:
    virtual void Update() override {
        ui::render_conan_logo();
        ...
        ImGui::Begin(
            "CustomImGui Hello, world!"); // Create a window called "Hello,
        ...
        bool clear_color_changed = ImGui::ColorEdit3(
            "clear color",
...
int main(int argc, char *argv[]) {
    ...
    if (!gladLoadGLLoader(
            (GLADloadproc)glfwGetProcAddress)) // tie window context to glad's
                                               // opengl funcs
