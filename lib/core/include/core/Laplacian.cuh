...
__global__ void LaplacianKernel(const cuda::PtrStep<T_in> src,
    ...
    if (x > 0 && x < cols - 1 && y > 0 && y < rows - 1) {
        lx = ::utils::abs<T_out>(src(y, x + 1) + src(y, x - 1) - 2 * src(y, x));
        ly = ::utils::abs<T_out>(src(y + 1, x) + src(y - 1, x) - 2 * src(y, x));
        dst(y, x) = lx + ly;
    }
}

std::tuple<float, float> gpuLaplacian(const Mat &h_oriImg) {
    ...
    float res = static_cast<float>(sum) / (rows * cols);
